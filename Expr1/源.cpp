		...
		while (true) {
			...
			if (len == -2) {
				...
				if (senderror >= RECV_LOOP_COUNT) {
					printf("[ERROR: 0x10] 传输失败：重传Request次数过多!\n");
					printTime(); fprintf(fp, "[ERROR: 0x10] 传输失败：重传Request次数过多!\n");
					return false;
				}
				printf("[WARNING: 0x90] 应答超时，重传Request!\n");
				len = sendRequest(RRQ, filename, type);
			}
			else if (len == -1) {
				printf("[ERROR: %d]传输失败.\n", WSAGetLastError());
				printTime(); fprintf(fp, "[ERROR: %d]传输失败.\n", WSAGetLastError());
				return false;
			}
				...
				if (blockNum + 1 == recvnum) {//正常块
					...
					if (errortimes < RECV_LOOP_COUNT) {
						...
						printf("[ERROR: 0x11] 文件传输失败：未收到正确数据包！\n");
						printTime(); fprintf(fp, "[ERROR: 0x11] 文件传输失败：未收到正确数据包！\n");
						return false;
					}
				}
				if (len < 516) break;
...
bool pushFile(const char filename[], int type) //type=1为文本格式(txt)，2为二进制文件
{
	...
	if (type == 2) {
		...
		if (readfp == NULL) {
			printf("[ERROR: 0x12] 传输失败：未能打开此文件\n");
			printTime(); fprintf(fp, "[ERROR: 0x12] 传输失败：未能打开此文件\n");
			exit(4);
		}
		...
		while (true) {
			...
			if (len == -2) {
				senderror++;
				if (senderror >= RECV_LOOP_COUNT) {
					printf("[ERROR: 0x11] 传输失败：重传Request次数过多!\n");
					printTime(); fprintf(fp, "[ERROR: 0x11] 传输失败：重传Request次数过多!\n");
					return false;
				}
				printf("[WARNING: 0x90] 应答超时，重传Request!\n");
